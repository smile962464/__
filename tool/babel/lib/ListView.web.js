...
class ListView extends React.Component {
  ...
  scrollTo(...args) {
    this.refs[SCROLLVIEW_REF] && this.refs[SCROLLVIEW_REF].scrollTo && this.refs[SCROLLVIEW_REF].scrollTo(...args);
  }
  ...
  render() {
    ...
    for (let sectionIdx = 0; sectionIdx < allRowIDs.length; sectionIdx++) {
      ...
      if (this.props.renderSectionHeader) {
        let shouldUpdateHeader = rowCount >= this._prevRenderedRowsCount && dataSource.sectionHeaderShouldUpdate(sectionIdx);
      ...
      for (let rowIdx = 0; rowIdx < rowIDs.length; rowIdx++) {
        ...
        let shouldUpdateRow = rowCount >= this._prevRenderedRowsCount && dataSource.rowShouldUpdate(sectionIdx, rowIdx);
        let row = React.createElement(StaticRenderer, {
          key: 'r_' + comboID,
        ...
        if (this.props.renderSeparator && (rowIdx !== rowIDs.length - 1 || sectionIdx === allRowIDs.length - 1)) {
          let adjacentRowHighlighted = this.state.highlightedRow.sectionID === sectionID && (this.state.highlightedRow.rowID === rowID || this.state.highlightedRow.rowID === rowIDs[rowIdx + 1]);
          let separator = this.props.renderSeparator(sectionID, rowID, adjacentRowHighlighted);
          if (separator) {
  ...
  _pageInNewRows() {
    this.setState((state, props) => {
      let rowsToRender = Math.min(state.curRenderedRowsCount + props.pageSize, props.dataSource.getRowCount());
